...
def create_parser(asciidoc=False):
    ...
    parser.add_option("", "--display-file-Path",
                      action="store_true", dest="displayFilePath", default=False,
                      help="By default, displays full files paths (instead of just file name). Use '--display_path' with runner application for dynamic display of paths.")
...
def parseCommandline(args):
    ...
    if options.xunit_printer or options.runner == "XUnitPrinter":
        ...
        options.xunit_file = re.sub("\\\\", "\\\\\\\\", options.xunit_file)  # This is mandatory for storing xunit file outside of current directory on Windows
#+        print ("options.xunit_file: {0}".format(options.xunit_file))
...
def writeHeaders( output ) :
    """ Writes optional and mandatory includes """
...
def writeMain( output ):
    ...
    lines = []
    lines += ['int %s( int argc, char *argv[] )' % options.main]
    lines += ['{']
    lines += ['    int status;']
    if options.noStaticInit:
        lines += ['    CxxTest::initialize();']
    ...
    if options.xunit_printer:
       lines += ['    std::ofstream ofstr("%s");' % options.xunit_file]
       lines += ['    %s tmp(ofstr);' % tester_t]
    else:
       lines += ['    %s tmp;' % tester_t]

    lines += ['']
    lines += ['    CxxTest::RealWorldDescription::_worldName = "%s";' % options.world]
    lines += ['    status = CxxTest::Main< %s >( tmp, argc, argv );'  % tester_t]
    lines += ['    return status;']
    lines += ['}']
    lines += ['']
...
def writeWorld( output ):
    ...
    if options.root or not options.part:
        ...
        writeDisplayFullPathName ( output )
    if options.noStaticInit:
...
def writeDisplayFullPathName( output ) :
    """
    " Write assignment of static variable _displayFilePath
    """
    output.write( 'bool CxxTest::RealWorldDescription::_displayFilePath = %s;\n' % ("true" if options.displayFilePath else "false"))
...
def writeWorldDescr( output ):
    ...
    if options.noStaticInit:
        ...
        output.write( 'const char* CxxTest::RealWorldDescription::_worldName      = "%s";\n' % options.world )
