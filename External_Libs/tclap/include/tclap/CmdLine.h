...
class CmdLine : public CmdLineInterface
{
  protected:
    ...
    bool _shouldExit = false;   //!< Report whether application should exit (error(s), just requesting version...)

    bool _automaticExit = true; //!< Enables/Disable automatic application exit
...
private:
  ...
  public:
    ...
    //!
    bool  automaticExit() const { return _automaticExit; }

    //! Returns whether parsing does automatic application exit when needed
    //!
    void  automaticExit (bool automaticExit) { _automaticExit = automaticExit; }
...
inline void CmdLine::_constructor()
{
  ...
  if (_helpAndVersion)
  {
    ...
    SwitchArg* help = new SwitchArg("h",
                                    "help",
...
{
  if (args.empty()) // Must be check to avoid application crash !
  {
    throw CmdLineParseException("Invalid, empty command line - must have at least program name -", "");
  }
