...
class DLL_EXPORT SPI_Player : public AccessInterfaceProtocol
{
  ...
  SPI_Player(std::initializer_list<uint32_t> readCommands, std::initializer_list<uint32_t> writeCommands, std::experimental::string_view commandsPrefix = "");
  SPI_Player(std::vector<uint32_t>           readCommands, std::vector<uint32_t>           writeCommands, std::experimental::string_view commandsPrefix = "");
