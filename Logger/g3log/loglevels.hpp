...
struct LEVELS
{
   ...
   int value;             //!< Level integral value
   std::string text;      //!< Textual representation of level
};
...
namespace g3
{
   static constexpr int kDebugValue = 0;
}
...
#endif
const LEVELS INFO      {g3::kDebugValue + 1, {"INFO"}};
const LEVELS WARNING   {INFO.value      + 1, {"WARNING"}};
const LEVELS ERROR_LVL {WARNING.value   + 1, {"ERROR"}};    // Cannot name it just ERROR because it create a compiler error!
...
namespace g3
{
   namespace internal
   {
      ...
      const LEVELS FATAL_SIGNAL    {1001, { "FATAL_SIGNAL"}};
      const LEVELS FATAL_EXCEPTION {1002, { "FATAL_EXCEPTION"}};
   ...
   namespace only_change_at_initialization {
      ...
      G3LOG_DLL_EXPORT void        setLogLevel(const LEVELS& level, bool enabled_status);
      G3LOG_DLL_EXPORT std::string printLevels();
      G3LOG_DLL_EXPORT void        reset();

   } // only_change_at_initialization
#endif

   G3LOG_DLL_EXPORT bool logLevel(const LEVELS& level);
} // g3
