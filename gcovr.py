    ...
    <tr>
      <td width="100%">
        <table cellpadding=1 border=0 width="100%">
          <tr>
            <td class="headerName">Directory:</td>
            <td class="headerValue">${DIRECTORY}</td>
          </tr>
          <tr>
            <td width="10%" class="headerName">Date:</td>
            <td width="15%" class="headerValue">${DATE}</td>
            <td width="15%"></td>
            <td width="30%"></td>
            ...
            <td width="10%" class="headerValue" style="text-align:right;">Coverage</td>
          </tr>
          <tr>
            <td class="headerName">Time:</td>
            <td class="headerValue">${TIME}</td>
            <td></td>
...
<body>
    ...
    <tr>
      <td width="100%">
        <table cellpadding="1" border="0" width="100%">
          ...
          <tr>
            <td width="10%" class="headerName">File:</td>
            <td width="15%" class="headerValue">${RELATIVE_FILENAME}</td>
            <td width="15%"></td>
            <td width="30%"></td>
            <td width="10%" class="headerValue" style="text-align:right;">Exec</td>
            <td width="10%" class="headerValue" style="text-align:right;">Total</td>
  ...
  <table cellspacing="0" cellpadding="1">
    <tr>
      <td width="3%"  align="right" class="srcHeader">Line</td>
      <td width="3%"  align="right" class="srcHeader">Branch</td>
      <td width="3%"  align="right" class="srcHeader">Exec</td>
      <td width="91%" align="left"  class="srcHeader src">Source</td>
    </tr>
...
class CoverageData(object):
    ...
    def relativePath(self):
        return options.root_filter.sub('', self.fname)

    def relativePathLength(self):
        return len(self.relativePath())

    def summary(self, maxPathsLength):
        ...
        if not self.fname.endswith(tmp):
            ...
            tmp = self.fname
        tmp = tmp.ljust(maxPathsLength)
        if len(tmp) > maxPathsLength:
            tmp = tmp + "\n" + " " * maxPathsLength
        ...
        lineText = tmp + str(total).rjust(8) + str(cover).rjust(8) + percent.rjust(6) + "%".ljust(4) + uncovered_lines
        return (total, cover, lineText)
...
#+    return os.path.join(os.path.relpath(os.path.sep.join(common_dirs)), '')
    return os.path.sep.join(common_dirs)
...
#
def process_gcov_data(data_fname, covdata, source_fname):
    print ("[process_gcov_data] GCov file path: ", data_fname)
...
#+    fnameFromRoot = os.path.relpath(fname, root_dir)

    # Finding source file corresponding to a gcov data file
    if options.verbose_debug:
        print('  currdir:          ' + currdir)
        print('  gcov_fname:       ' + data_fname)
        print('  1st:line segments ' + str(segments))
        print('  source_fname:     ' + source_fname)
        print('  root:             ' + root_dir)
        print('  fname:            ' + fname)
#+        print('  fnameFromRoot:    ' + fnameFromRoot)
        print('  common_dir:       ' + common_dir)

    if options.verbose:
        print ("  Parsing gcov data")
    #
    ...
    if filtered_fname is None:
        if options.verbose:
            sys.stdout.write("[process_gcov_data] Filtering out coverage data for file %s\n" % fname)
        return
    #
    ...
    for exc in options.exclude:
        if (filtered_fname is not None and exc.match(filtered_fname)) or \
            ...
            if options.verbose:
                sys.stdout.write( "[process_gcov_data] Excluding coverage data for file %s\n" % fname )
            return
    ...
    for line in INPUT:
#+        print ("line: ", line)
        segments = line.split(":", 2) # Try to make 3 segments out of gcov line (e.g. "        -:   61:using std::regex;")
#+        print("\tY", segments)
        firstSegment = segments[0].strip()
        if len(segments) > 1:
            try:
                lineno = int(segments[1].strip())
            except:
                print ("  Failed to get line number (after line number %d); segments: %s" % (lineno, str(segments)))
            ...
            for header, flag in exclude_line_pattern.findall(line):
                if flag == 'START':
                    ...
                    if excluding:
                        ...
                        if _header != header:
                            sys.stderr.write(
                                "  (WARNING) %s_EXCL_START found on line %s was terminated by %s_EXCL_STOP on line %s, when processing %s\n"
        ...
        if len(firstSegment) != 0:
          if firstSegment[0] == '-' or (excluding and firstSegment[0] in "#=0123456789"):
              ...
              is_code_statement = True
              uncovered_exceptional.append(lineno)
          elif firstSegment[0] in "0123456789":
              is_code_statement = True
              covered[lineno] = int(segments[0].strip())
          elif firstSegment.startswith('branch'):
              ...
              if exclude_branch:
                  if options.verbose:
                      print ("  Excluding unreachable branch on line %d (%s); Parsed line: %s" % (lineno, exclude_reason, line.strip()))
              else:
                  fields = line.split()
                  try:
                      count = int(fields[3])
          ...
          elif firstSegment.startswith('call'):
              pass
          elif firstSegment.startswith('function'):
              pass
          elif firstSegment[0] == 'f':
              pass
          else:
              sys.stderr.write(
                  "  (WARNING) Unrecognized GCOV output: '%s'\n"
                  "  \tThis is indecisive of a gcov output parse error.\n"
                  "  \tPlease report this to the gcovr developers." % firstSegment
              )
    ...
    if options.verbose:
        if    uncovered:             print('  Uncovered lines:       %d ==> %s' % (len(uncovered), str(uncovered)))
        else:                        print('  None uncovered')
        if    uncovered_exceptional: print('  Uncovered (exception): %d ==> %s' % (len(uncovered_exceptional), str(uncovered_exceptional)))
        #print('covered ',+covered)
        #print('branches ',+str(branches))
        #print('noncode ',+str(noncode))

    #
    # If the file is already in covdata, then we remove lines that are covered here. Otherwise, initialize covdata
    #
    if not fname in covdata:
        covdata[fname] = CoverageData(fname, set(uncovered), set(uncovered_exceptional), covered, branches, set(noncode))
    else:
        covdata[fname].update(set(uncovered), set(uncovered_exceptional), covered, branches, set(noncode))
    INPUT.close()

#+    print ("INPUT file closed: ", data_fname)
...
#
def process_datafile(filename, covdata):
    if options.verbose:
        print("[process_datafile] File: ", filename)
    #
    # Launch gcov
    #
    ...
    if len(potential_wd) == 0:
        ...
        while True:
            ...
            if wd == potential_wd[-1]:
                break # Stop at the root of the file system
    ...
    while len(potential_wd) > 0 and not Done:
        ...
        if options.verbose_debug:
            print ("  Find the files that gcov created")
        ...
        for line in out.splitlines():
            ...
            if found is not None:
                ...
                if not options.gcov_filter.match(fname):
                    ...
                    if options.verbose:
                        print ("  Filtering out gcov file: ", fname)
                    continue
        ...
        if source_re.search(err):
            errors.append(err)  # gcov tossed errors: try the next potential_wd
        else:
            #
            # Process *.gcov files
            #
            for fname in gcov_files['active']:
                if options.verbose:
                    print ("  Processing active file:  ", fname)

                process_gcov_data(fname, covdata, abs_filename)
            Done = True
            ...
            for group in gcov_files.values():
                for fname in group:
                    if os.path.exists(fname):
                        os.remove(fname)  # Only remove files that actually exist.
...
#
def process_existing_gcov_file(filename, covdata):
    #
    ...
    #
    process_gcov_data(filename, covdata, None)
    #
...
def print_text_report(covdata):
    if options.verbose:
        ...
        print ("\n")
        print ("=================================================================================")
        print ("============================ Printing Tabular Report ============================")
        print ("=================================================================================")
        print ("\n")
    else: print ("Printing Tabular Report")
    ...
    def _printHeaderSeparator():
      OUTPUT.write("-" * headersWidth + '\n')

    def _printCentered(text):
      textWidth = len(text)
      spaces    = " " * int((headersWidth - textWidth) / 2)
      OUTPUT.write("%s%s%s\n" % (spaces, text, spaces))

    # ---------------- Compute columns count needed for paths
    #
    maxPathsLength = 20
    for key in covdata.keys():
      maxPathsLength = max(maxPathsLength, covdata[key].relativePathLength())

    maxPathsLength += 2 # Leave some room before next field
    ...
    _printHeaderSeparator()
    linesOrBranch = options.show_branch and "Branches" or "Lines"
    takenOrExec   = options.show_branch and "Taken" or "Exec"
    OUTPUT.write("File".ljust(maxPathsLength) + linesOrBranch.rjust(8) + takenOrExec.rjust(8) + "Cover".rjust(7) + "   Missings lines\n")
    _printHeaderSeparator()
    ...
    for key in keys:
        (t, n, txt) = covdata[key].summary(maxPathsLength)
        total_lines += t
        total_covered += n
        OUTPUT.write(txt + '\n')
    ...
    OUTPUT.write("TOTAL".ljust(maxPathsLength) + str(total_lines).rjust(8) + str(total_covered).rjust(8) + str(percent).rjust(6) + "%\n")
    _printHeaderSeparator()
...
# Updates source files path to have clean absolute path and path relative to common path
def tidyUpSourceFilesPath(covdata):

    if options.verbose:
        print ("tidyUpSourceFilesPath:")
        print ("  root_dir:       ", root_dir)

    keys           = list(covdata.keys())
    commonPath     = os.path.commonpath(keys) if keys else ""
    files          = []
    fullPaths      = []
    dirs           = []
    filtered_fname = ''
...
#+    keys.sort( key = options.sort_uncovered and _num_uncovered or options.sort_percent and _percent_uncovered or _alpha )
    ...
    for fullPath in keys:
        pathFromCommon = os.path.relpath(fullPath, commonPath)
        pathFromRoot = options.root_filter.sub('', fullPath)
        dirFromRoot  = os.path.dirname(pathFromRoot) + os.sep

        dirs.append(dirFromRoot)
        files.append(pathFromRoot)
        fullPaths.append(fullPath)

        if options.verbose:
            print ("  fullPath:       ", fullPath)
            print ("  dirFromRoot:    ", dirFromRoot)
            print ("  pathFromRoot:   ", pathFromRoot)
            print ("  pathFromCommon: ", pathFromCommon)

#+        cdata        = covdata[fullPath]
#+        cdata._filename = fullPath

#+    commonPath = commonpath(fullPaths)
    ...
    return commonPath
#
#  End of function 'tidyUpSourceFilesPath'
#---------------------------------------------------------------------------
...
def print_html_report(covdata, details):
    if options.verbose:
        ...
        print ("==============================================================================")
        print ("\n")
    else: print ("Printing Html Report")
    ...
    outputOption = os.path.abspath(options.output)
    outputDir    = outputOption;
    outputParts  = outputOption.split('.')
    (base, ext)  = os.path.splitext(outputDir)
    if ext:
    ...
    header = str("CCov: %s") % (os.path.basename(options.root) if (options.root) else (os.path.basename(root_dir)))
    ...
    for f in keys:
        ...
        while True:
            if len(outputParts) > 1:
                ...
                cdata._sourcefile = \
                    outputParts[0] + '.' + longname + longname_hash + '.html'
    ...
    for f in keys:
        ...
        if options.verbose_debug:
            ...
            print ("filenameAbsolutePath: ", filenameAbsolutePath)
            print ("filenameFromRoot:     ", filenameFromRoot)
            print ("outputFileBaseName:   ", outputFileBaseName)
            print ("outputFilePath:       ", outputFilePath)

        row = html_row(details,
                       cdata._sourcefile,
                       directory          = data['DIRECTORY'],
                       filename           = filename,
...
#+            print ("row:   ", row)
        ...
        cdata._filename          = filenameAbsolutePath
        cdata._sourcefile        = outputFilePath
        cdata._sourcefileRelPath = filenameFromRoot
    ...
    #
    if options.verbose:
        print ("Generating HTML file for every source file")

    for f in keys:
        cdata = covdata[f]

        data['FILENAME']          = cdata._filename
        data['RELATIVE_FILENAME'] = cdata._sourcefileRelPath
        data['HEAD'] = str("CCov: %s") % os.path.basename(cdata._filename)
        ...
        inputFilePath  = data['FILENAME']
        outputFilePath = cdata._sourcefile

        if options.verbose:
          print ("Input file (Rel): ", data['RELATIVE_FILENAME'])
          print ("Input file:       ", inputFilePath)
          print ("HTML Output file: ", outputFilePath)

        if not Path(inputFilePath).is_file():
          print ("Cannot find file: ", inputFilePath)
        else:
...
def html_row(details, sourcefile, **kwargs):
    ...
    if nrows % 2 == 0:
        ...
        kwargs['altstyle'] = ''
    if details:
        kwargs['filename'] = '<a href="%s">%s</a>' % (kwargs['outputFileBaseName'], kwargs['filename'])
...
def GetOptions():
  ...
  parser.add_option(
      "-V", "--verbose_debug",
      help="Print progress messages for debug",
      action="store_true",
      dest="verbose_debug",
      default=False
  )
  ...
  parser.add_option(
      "-t", "--print-tabular",
      help="Prints a standard tabular report to stdout with line & branch "
           "percentage coverage (even when xml or html print is requested)",
      action="store_true",
      dest="print_tabular",
      default=False
  )
...
def main():
  ...
  #
  print ("gcovr: " + datetime.now().strftime('%H:%M:%S'))
  print ("Need Python 3.5 or greater to work properly !!!")
...
#+  sys.exit(0)
  ...
  if options.verbose:
      print ("options.gcov_exclude: ", options.gcov_exclude)
  ...
  if len(args) == 1:
      ...
      datafiles = get_datafiles(args[1:])


  print("Have collected %d data files" % len(datafiles))
  ...
  for file_ in datafiles:
      if options.gcov_files:
          process_existing_gcov_file(file_, covdata)
      else:
  ...
  if printTabular or options.print_tabular:
