...
namespace g3
{
  bool g_logIsEnabled = true;

  //! Enables/Disables LOG, LOG_IF, LOGF and LOGF_IF globally.
  //! @note  No sinks have the opportunity to see any log when it is disabled
  bool logEnabled(bool newState)
  {
    auto oldState = g_logIsEnabled;
    g_logIsEnabled = newState;
    return oldState;
  }

  //! Returns whether LOG, LOG_IF, LOGF and LOGF_IF are globally enabled or disabled.
  //!
  bool logEnabled() { return g_logIsEnabled; }

  LEVELS g3::FunctionScopeLogger::Level = INFO;

  //! Logs that function is returning (for whatever reason)
  //!
  g3::FunctionScopeLogger::~FunctionScopeLogger()
  {
    LogMessage::leaveBlock();
    if(g3::logEnabled() && g3::logLevel(Level))
    {
      LogCapture logCapture(m_fileName, m_line, m_functionName, Level);
      auto& os = logCapture.stream();

      os << LOG_BLOCK_LEAVE_PREFIX << m_functionName;
      auto returnMessage = m_osReturnMessage.str();
      if (!returnMessage.empty())
      {
        os << "(Returns: " << returnMessage << ")";
      }
      os << LOG_BLOCK_LEAVE_SUFFIX;
    }
  }

  g3::FunctionScopeLogger::FunctionScopeLogger(const char* fileName, int line, const char* functionName)
  : m_fileName     (fileName)
  , m_line         (line)
  , m_functionName (functionName)
  {
    if(g3::logEnabled() && g3::logLevel(Level))
    {
      ...
      LogCapture logCapture(m_fileName, m_line, m_functionName, Level);
      auto& os = logCapture.stream();
      os << LOG_BLOCK_ENTER_PREFIX << m_functionName << LOG_BLOCK_ENTER_SUFFIX;
    }
    LogMessage::enterBlock();
  }


  //! Streams a comma before second and following parameters
  //!
  void g3::FunctionScopeLogger::ManageSeparator (std::ostringstream& os)
  {
    if (!m_firstParameter)
    {
      os << ", ";
  ...
  void g3::FunctionScopeLogger::StreamParameters (std::ostringstream& os, const char* text)
  {
    ManageSeparator(os);
    os << "\"" << text << "\""  ;
  }
  ...
  //
  void G3LOG_DLL_EXPORT initializeLogging(LogWorker *bgworker)
  {
  ...
  * */
  ...
  namespace internal
  {
    ...
    /**
     * Shutdowns the logging by making the pointer to the background logger to nullptr. The object is not deleted
    ...
    {
       if (isLoggingInitialized() && (active != nullptr)  && (active != g_logger_instance))
       {
  ...
  {
    LogCapture(file, line, function, level).stream() << instanceName << ".hasMatch(): " << std::boolalpha << instance.hasMatch();

    auto lastCapturedIndex = instance.lastCapturedIndex();
    for (int ii = 0 ; ii <= lastCapturedIndex ; ++ii)
    {
      LogCapture(file, line, function, level).stream() << "group[" << ii << "]: '" << instance.captured(ii) << "'";
    }
  }
  #endif
...
{
  os << "QRegularExpressionMatch::hasMatch: " << std::boolalpha << matches.hasMatch();

  auto lastCapturedIndex = matches.lastCapturedIndex();
  for (int ii = 0 ; ii <= lastCapturedIndex ; ++ii)
  {
    os << "\ngroup[" << ii << "]: '" << matches.captured(ii) << "'";
  }
