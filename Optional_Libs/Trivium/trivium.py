...
class Trivium:
	...
	self.key = hex_to_bits(key)[::-1]  # self._setLength(key)
        self.iv = hex_to_bits(iv)[::-1] # self._setLength(iv)
    	
	#print ("Initializing Trivium with Key "+key+" and IV "+iv)
        # Initialize state
        # len 100
    ...
    # Check python version
        if version_info[0] == 2:
            next_key_bit = self.keystream().next
        ...
        for j in range(nbits):
            new_keystream.append(next_key_bit())
#        print(bits_to_hex(new_keystream))
	return bits_to_hex_big(new_keystream)    
...
def doubleprint(message1,message2):
...
#    IV = hex_to_bits("288FF65DC42B92F960C7")[::-1]
    ...
    nbits =16
    print("Generating a keystream of "+str(nbits)+" bits")
    key_out= trivium.get_keystream(nbits)
...
def bits_to_hex_big(b):
    return "".join(["%02X" % sum([b[(i+8) - j] << (j-1) for j in range(1,9)])
                    for i in range(0, len(b), 8)])
