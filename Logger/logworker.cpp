...
namespace g3 {
   ...
   //! Stops background thread, then sinsk thread
   //!
   // From code previously in LogWorker destructor:
   //
   ...
   //
   // The explicitly clearing of the background worker (below) makes sure that there can
   ...
   LogWorker::~LogWorker()
   {
      g3::logEnabled(false);  // Stops all NEW logs
   ...
   std::unique_ptr<FileSinkHandle>LogWorker::addDefaultLogger(const std::string& log_prefix, const std::string& log_directory) {
      return addSink(std::make_unique<g3::FileSink>(log_prefix, log_directory), &FileSink::fileWrite);
   }
} // g3
