  ...
  try
    {
      ...
      cout<<"ID 0x"<<hex<<ID.to_ulong()<<" Current_Ctl 0x"<<hex<<Current_Ctl.to_ulong()<<" ScanData 0x"<<hex<<ScanData.to_ulong()<<" ScanCtl 0x"<<hex<<ScanCtl.to_ulong()<<"\n";
		  ...
		  switch (opcode)
		   {
		    ...
		    case DATA:
		               ...
		               if (Current_Ctl == ScanData_Addr) {test_packet.data=shift_8bit_dirty(ScanData,test_packet.data);}
			       if (Current_Ctl == ScanCtl_Addr)  {ScanCtl =tmp ;test_packet.data=ScanCtl;}
		    	      break;
		    default : break;
		   }
