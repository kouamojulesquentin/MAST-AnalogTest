   ...
   private:
      ...
      void run()
      {
        try
        {
          ...
          COUT_LOG_TIMESTAMP << std::endl << "Active object @ 0x" << std::hex << this << " is leaving" << std::endl;
        }
        catch(std::exception& exception)
        {
          CERR_LOG_TIMESTAMP << "In Active object @ 0x" << std::hex << this << " in " << __func__ << " got std::exception: " << exception.what();
        }
        catch (...)
        {
          CERR_LOG_TIMESTAMP << "In Active object @ 0x" << std::hex << this << " in " << __func__ << " got unknown exception";
        }
      }

      shared_queue<Callback> mq_;
      std::thread            thd_;
      volatile bool          done_;   // Volatile to be sure that reading it in thread loop is not optimized (using register)
   public:
      virtual ~Active()
      {
        stop();
      ...
      void stop()
      {
        if (thd_.joinable())
        {
          auto action = [this] { done_ = true; };
          send(action);
          thd_.join();
          COUT_LOG_TIMESTAMP << std::endl << "Active object @ 0x" << std::hex << this << " has been joined" << std::endl;
        }
      }
