...
//! Makes QString serializable into and ostringstream
//!
inline std::ostream& operator<<(std::ostream& os, const QString& text) { auto asAsciiString = text.toLatin1().constData(); os << asAsciiString; return os; }

//! Makes QRegularExpressionMatch serializable into and ostringstream
//!
std::ostream& operator<<(std::ostream& os, const QRegularExpressionMatch& matches);

namespace g3
{
  //! Properly logs captured groups of a QRegularExpressionMatch
  //!
  void logObject(const char* file, const int line, const char* function, const LEVELS& level, const char* instanceName, const QRegularExpressionMatch& instance);
} // End of namespace g3
...
namespace g3
{
  ...
  //! Enables/Disables LOG and LOGF globally.
  //! @note  No sinks have the opportunity to see any log when it is disabled
  bool G3LOG_DLL_EXPORT logEnabled(bool newState);

  //! Returns whether LOG and LOGF are globally enabled or disabled.
  //!
  bool G3LOG_DLL_EXPORT logEnabled();
  ...
  */
  ...
  #define LOG_BLOCK_ENTER_PREFIX "-->> "
  #define LOG_BLOCK_ENTER_SUFFIX " >>----"
  #define LOG_BLOCK_LEAVE_PREFIX "--<< "
  #define LOG_BLOCK_LEAVE_SUFFIX " <<----"


  class G3LOG_DLL_EXPORT FunctionScopeLogger final
  {
    const char* const  m_fileName;
    const int          m_line;
    const char* const  m_functionName;
    bool               m_firstParameter = true;
    std::ostringstream m_osReturnMessage;
    ...
    FunctionScopeLogger(const char* fileName, int line, const char* functionName);

    template<typename... TArgs>
    FunctionScopeLogger(const char* fileName, int line, const char* functionName, TArgs... args)
    : m_fileName     (fileName)
    , m_line         (line)
    , m_functionName (functionName)
    {
      if(g3::logEnabled() && g3::logLevel(Level))
      {
        m_osReturnMessage << std::boolalpha;

        LogCapture logCapture(m_fileName, m_line, m_functionName, Level);
        auto& os = logCapture.stream();
        os << std::boolalpha;

        os << LOG_BLOCK_ENTER_PREFIX << m_functionName << "(";
        StreamParameters(os, args...);
        os << ")" LOG_BLOCK_ENTER_SUFFIX;
      }
      LogMessage::enterBlock();
    }

    std::ostringstream& Stream() { return m_osReturnMessage; };

    private:
    void ManageSeparator  (std::ostringstream& os);
    void StreamParameters (std::ostringstream& os, const char* text);
    void StreamParameters (std::ostringstream& os, const std::string& text)
    {
      StreamParameters(os, text.c_str());
    }

    template<typename T>
    void StreamParameters(std::ostringstream& os, const T& arg)
    {
      ManageSeparator(os);
      os << arg;
    }
    ...
    void StreamParameters(std::ostringstream& os, const T& firstArg, TArgs... args)
    {
      StreamParameters(os, firstArg);
      StreamParameters(os, args...);
    }
  };
  ...
  //! Default implementation for logging unknown type instance
  //!
  //! @note An overload should be added (in g3 namespace) for each type to support
  template<typename T>
  void logObject(const char* file, const int line, const char* function, const LEVELS& level, const char* instanceName, const T& /* instance */)
  {
    LogCapture(file, line, function, level).stream() << "Due to unsupported type, cannot log instance '"
                                                     << instanceName
                                                     << "' (you should implement g3::logObject function overload for this type)";
  }

  inline bool canLog(const LEVELS& level) { return g3::logEnabled() && g3::logLevel(level); }
} // g3
...
#define LOG(level) IF_CAN_LOG(level) INTERNAL_LOG_MESSAGE(level).stream()

#define LOG_FUNCTION_SCOPE       g3::FunctionScopeLogger functionScopeLogger(__FILE__, __LINE__, __func__)
#define LOG_FUNCTION_SCOPE_(...) g3::FunctionScopeLogger functionScopeLogger(__FILE__, __LINE__, __func__, __VA_ARGS__);
#define LOG_FUNCTION_RETURN      IF_CAN_LOG(g3::FunctionScopeLogger::Level) functionScopeLogger.Stream()
#define LOG_OBJECT(level, instance)   IF_CAN_LOG(level) g3::logObject(__FILE__, __LINE__, __func__, level, #instance, instance);

// 'Conditional' stream log
#define LOG_IF(level, boolean_expression)  \
   if(true == boolean_expression)  \
      if(g3::logEnabled() && g3::logLevel(level))  INTERNAL_LOG_MESSAGE(level).stream()
...
#define LOGF(level, printf_like_message, ...)                 \
   if(g3::logEnabled() && g3::logLevel(level)) INTERNAL_LOG_MESSAGE(level).capturef(printf_like_message, ##__VA_ARGS__)
