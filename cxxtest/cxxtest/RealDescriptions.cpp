...
bool RealWorldDescription::_displaySuccess = false;
...
RealTestDescription::RealTestDescription(List &argList, SuiteDescription &argSuite, unsigned argLine, const char *argTestName)
  : _suite             (0)
  , _line              (0)
  , _testName          (0)
  , _isDataDriven      (false)
  , _testId            (0)
  , _inputSampleAsText (0)
{
    initialize(argList, argSuite, argLine, argTestName);
}

void RealTestDescription::initialize(List &argList, SuiteDescription &argSuite, unsigned argLine, const char *argTestName)
{
    _suite             = &argSuite;
    _line              = argLine;
...
bool RealTestDescription::setUp()
{
    ...
    for (GlobalFixture *gf = GlobalFixture::firstGlobalFixture(); gf != 0; gf = gf->nextGlobalFixture())
    {
        _TS_TRY
        {
          bool success = gf->setUp();
          if (!success)
          {
            _TS_FAIL (file(), line(), "Got failure status for some GlobalFixture::setUp()");
            return false;
          }
        }
        _TS_LAST_CATCH ( { tracker().failedAssertThrowsNothing("Got unexpected exception for some GlobalFixture::setUp()", file(), line(), "gf->setUp()", NULL); return false; })
    }
    ...
    _TS_LAST_CATCH    ( { tracker().failedAssertThrowsNothing("", file(), line(), "suite()->setUp()", NULL); return false; })
...
bool RealTestDescription::tearDown()
{
    ...
    _TS_CATCH_ABORT   ( { return false; })
    _TS_CATCH_SKIPPED ( { return false; })
    _TS_LAST_CATCH    ( { tracker().failedAssertThrowsNothing("", file(), line(), "suite()->tearDown()", NULL); return false; })

    for (GlobalFixture *gf = GlobalFixture::lastGlobalFixture(); gf != 0; gf = gf->prevGlobalFixture())
    {
        _TS_TRY
        {
          bool success = gf->tearDown();
          if (!success)
          {
            _TS_FAIL (file(), line(), "Got failure status for some GlobalFixture::tearDown()");
            return false;
          }
        }
        _TS_LAST_CATCH ( { tracker().failedAssertThrowsNothing("Got unexpected exception for some GlobalFixture::tearDown()", file(), line(), "gf->tearDown()", NULL); return false; })
    }
...
void RealTestDescription::run()
{
    ...
    _TS_CATCH_ABORT( {})
    _TS_CATCH_SKIPPED( {})
    _TS_CATCH_UNEXPECTED(file(), line());
}
