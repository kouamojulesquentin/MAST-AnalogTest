...
void AES_init_ctx(AES_struct* ctx, const u8* key)
{
...
#if (defined(CBC) && CBC == 1) || (defined(ECB) && ECB == 1)
static void InvCipher(state_t* state, const u8* RoundKey)
{
  u8 round = 0;
...
static void XorWithIv(u8* buf, const u8* Iv)
{
  u8 i;
...
void AES_CBC_encrypt_buffer(struct AES_ctx *ctx, u8* buf, uint32_t length)
{
  uintptr_t i;
  u8 *Iv = ctx->Iv;
...
void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, u8* buf,  uint32_t length)
{
  uintptr_t i;
...
void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, u8* buf, uint32_t length)
{
