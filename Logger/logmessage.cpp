...
thread_local int g3::LogMessage::sm_threadDepth = 0;

namespace
{
   std::once_flag                        g_start_time_flag;
   std::chrono::steady_clock::time_point g_start_time;
   //+ Todo (JFC) (October/20/2015) : make g_start_time atomic (if possible)
   //+
...
// http://stackoverflow.com/questions/3279543/what-is-the-copy-and-swap-idiom
   ...
   std::string LogMessage::threadID() const
   {
    ...
    //! Clears current log
    //!
    //! @note - Indent level is reset (for current thread)
    //!       - Timestamp is reset
    //!
   ...
   //! Increases thread associated (call) depth
   //!
   void LogMessage::enterBlock ()
   {
     ++sm_threadDepth;
   }

  //! Decreases thread associated (call) depth
  //!
  void LogMessage::leaveBlock ()
  {
    if (sm_threadDepth != 0)
    {
      --sm_threadDepth;
